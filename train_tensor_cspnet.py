...
def adjust_learning_rate(optimizer, epoch, initial_lr=1e-4, decay=1):
    optimizer.lr = initial_lr * (decay ** (epoch // 100))


def main(train, val, test, train_y, val_y, test_y, sub, total_sub, validation, train_batch_size=32,
         valid_batch_size=32, test_batch_size=32, use_cuda=True, mlp=True, 
         initial_lr=0.001, alg_name="Tensor_CSPNet", weights_folder_path='model_params/',
         patience=15, epochs=100, log_interval=1):
    ...
    model = Tensor_CSPNet_Basic(channel_num = train.shape[1],
        mlp = mlp,
        dataset = 'BCIC',
        ).to(device)

    optimizer = geoopt.optim.RiemannianAdam(model.parameters(), lr=initial_lr)

    early_stopping = EarlyStopping(
        alg_name = alg_name, 
        path_w   = weights_folder_path + alg_name + '_checkpoint.pt', 
        patience = patience, 
        verbose  = True, 
        )
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch_idx, (batch_train, batch_train_y) in enumerate(train_loader):
            ...
            loss   = F.nll_loss(output, batch_train_y)
            loss.backward()
            optimizer.step()
            ...
            if batch_idx % log_interval == 0:
                ...
                torch.save(model.state_dict(), weights_folder_path + alg_name+'_model.pth')
                torch.save(optimizer.state_dict(), weights_folder_path+'optimizer.pth')


                print('['+alg_name+': Sub No.{}/{}, Epoch {}/{}, Completed {:.0f}%]:\nTrainning loss {:.10f} Acc.: {:.4f}'.format(\
                        sub, total_sub, epoch, epochs, 100. * (1+batch_idx) / len(train_loader), loss.cpu().detach().numpy(),\
                        train_correct.item()/batch_train.shape[0])
                )
    ...
    with torch.no_grad():
        for batch_idx, (batch_test, batch_test_y) in enumerate(test_loader):
            ...
            print('test acc : ', test_correct.item()/len(test_loader.dataset))
