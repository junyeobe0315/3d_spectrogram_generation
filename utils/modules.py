...
class Graph_BiMap(nn.Module):
    ...
    def __init__(self,h,ni,no,P):
        ...
        self._W =functional.StiefelParameter(th.empty(self._h, self._ni, self._no, dtype=dtype,device=self.device))
        #self._W = geoopt.ManifoldParameter(th.empty(self._h, self._ni, self._no), manifold=geoopt.CanonicalStiefel())
    ...
    def forward(self, X):
        ...
        if self.increase_dim:
            return self._bimap_multiplication(self.increase_dim(th.matmul(self._P.to(self.device), 
                                                              X.reshape((batch_size, channel_num, -1))).reshape((batch_size, channel_num, dim, dim))))
        else:
            return self._bimap_multiplication(th.matmul(self._P.to(self.device), X.reshape((batch_size, channel_num, -1))).reshape((batch_size, channel_num, dim, dim)))
