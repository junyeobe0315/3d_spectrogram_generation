...
class load_BCIC:
    def __init__(self, sub=1, train_sub = None, valid_sub = None, test_sub = None,
                 TorE = True, alg_name ='Tensor_CSPNet', session_no = 1, scenario = 'CV'):
        ...
        if scenario == 'CV':
            ...
            self.train_indices = np.load('index/sess'+str(session_no)+'_sub'+str(sub)+'_train_index.npy', allow_pickle=True)
            self.test_indices  = np.load('index/sess'+str(session_no)+'_sub'+str(sub)+'_test_index.npy', allow_pickle=True)
        ...
        elif scenario == "subject-independent":
            self.train_x , self.train_y = [], []
            if valid_sub != None:
                self.valid_x, self.valid_y = [], []
            self.test_x, self.test_y = [], []
            
            for subject in train_sub:
                x1, y1 = get_data(subject, True, 'dataset/')
                x2, y2 = get_data(subject, False, 'dataset/')
                self.train_x.extend(x1)
                self.train_x.extend(x2)
                self.train_y.extend(y1)
                self.train_y.extend(y2)
            
            if valid_sub != None:
                for subject in valid_sub:
                    x1, y1 = get_data(subject, True, 'dataset/')
                    x2, y2 = get_data(subject, False, 'dataset/')
                    self.valid_x.extend(x1)
                    self.valid_x.extend(x2)
                    self.valid_y.extend(y1)
                    self.valid_y.extend(y2)

            for subject in test_sub:
                x1, y1 = get_data(subject, True, 'dataset/')
                x2, y2 = get_data(subject, False, 'dataset/')
                self.test_x.extend(x1)
                self.test_x.extend(x2)
                self.test_y.extend(y1)
                self.test_y.extend(y2)
            ...
            self.train_x = fbank.filter_data(self.train_x, window_details={'tmin':3.0, 'tmax':7.0}).transpose(1, 0, 2, 3)
            if valid_sub != None:
                self.valid_x = fbank.filter_data(self.valid_x, window_details={'tmin':3.0, 'tmax':7.0}).transpose(1, 0, 2, 3)
            self.test_x = fbank.filter_data(self.test_x, window_details={'tmin':3.0, 'tmax':7.0}).transpose(1, 0, 2, 3)
            
            self.train_x = self._tensor_stack(self.train_x)
            if valid_sub != None:
                self.valid_x = self._tensor_stack(self.valid_x)
            self.test_x = self._tensor_stack(self.test_x)
        
        elif scenario == "raw-signal-si":
            self.train_x , self.train_y = [], []
            if valid_sub != None:
                self.valid_x, self.valid_y = [], []
            self.test_x, self.test_y = [], []
    ...
    def generate_training_valid_test_set_subject_independent(self):
        if self.alg_name == 'Graph_CSPNet':
            self.lattice = np.mean(self.train_x, axis=0)
        if self.valid_sub != None:
